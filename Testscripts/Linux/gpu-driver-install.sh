...
function install_gpu_requirements() {
	...
	case $DISTRO in
		...
		ubuntu*)
			...
			if [ $? -eq 0 ]; then
				LogMsg "Successfully installed build-essential libelf-dev linux-tools linux-cloud-tools python libglvnd-dev"
			else
				LogErr "Failed to install build-essential libelf-dev linux-tools linux-cloud-tools python libglvnd-dev"
				SetTestStateAborted
				return 1
			fi
		;;
